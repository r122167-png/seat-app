...
def fits_in_room(layout, room):
    chair_w = room['chair_w']
    chair_d = room['chair_d']
    corridor = room['corridor']
    cols = max(layout)
    rows = len(layout)
    needed_w = cols * (chair_w + corridor)
    needed_d = rows * (chair_d + corridor)
    return needed_w <= room['width'] and needed_d <= room['depth']
...
with tab1:
    ...
    if room and n > calc_max_chairs(room):
        st.warning(f'⚠️ この部屋の最大椅子数（{calc_max_chairs(room)}脚）を超えています！')
    ...
    if st.button('配置を提案する', type='primary'):
        ...
        # 部屋が選ばれていたら収まる候補だけに絞る
        if room:
            layouts = [l for l in layouts if fits_in_room(l, room)]

        if not layouts:
            st.error('この部屋に収まる配置が見つかりませんでした。人数を減らすか、部屋の設定を確認してください。')
        else:
            results = sorted(
    ...
    if st.session_state.top3:
        ...
        choice = st.radio(
            '番号を選んでや',
            options=list(range(1, len(st.session_state.top3) + 1)),
